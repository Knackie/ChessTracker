function getByValue(map, searchValue) {
  ...
  var player;
  fetch(
    "https://raw.githubusercontent.com/Knackie/ChessTracker/main/players.json"
  )
    ...
    .then((p) => {
      ...
      fetch(
        "https://raw.githubusercontent.com/Knackie/ChessTracker/main/matches.json"
      )
        .then((response) => response.json())
        .then((data) => {
          var won = 0;
          ...
          for (let i = 0; i < sortedAsc.size; i++) {
            ...
            lineClassement +=
              " " + gamesWon.get(Array.from(sortedAsc.keys())[i]);
            lineClassement += " / ";
            lineClassement += gamesPlayed.get(
              Array.from(sortedAsc.keys())[i]
            );
